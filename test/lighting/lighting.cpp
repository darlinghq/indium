...
private:
	...
	void makePipeline() {
		...
		pipelineDescriptor.fragmentFunction = library->newFunction("fragment_light");
	...
	void makeResources() {
		auto model = std::make_shared<OBJModel>(teapot, teapot_len, true);
		auto group = model->groupForName("teapot");
